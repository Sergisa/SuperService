...
def make_post_request(api_path='', data=None):
    return requests.get(url+api_path, cookies=coockies, headers=headers, data=json.dumps(data)).json()
...
def get_applications(page=1, search_fullname=None, items_per_page=100, status=None):
    ...
    if search_fullname != None:
        link += '&search_fullname={0}'.format(search_fullname)
    return make_get_request(link)


def set_status(id, status):
    result_reject = make_post_request('{0}/status/set'.format(id), data={
        'code': "app_call_off",
        'notification': None,
        'status_comment': None
    })
